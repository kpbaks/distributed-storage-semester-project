...
def replicate(file_data, max_erasures, k, send_task_socket, response_socket):
    """
    Store a file using Reed Solomon erasure coding, protecting it against 'max_erasures' 
    unavailable storage nodes. 
    The erasure coding part codes are the customized version of the 'encode_decode_using_coefficients'
    ...
    for j in range(k):
        for i in range(STORAGE_NODES_NUM):
            ...
            encoder.encode_symbol(symbol, coefficients[:symbols])
            # Generate a random name for it and save
            name = random_string(8)+"."+str(j)+"."+str(i)
...
def decode_file(symbols: List[bytes]) -> bytes:
    ...
    assert isinstance(symbols, list), f"symbols must be a list, not {type(symbols)}"
...
def get_file(coded_fragments: List[str], max_erasures: int, file_size: int,
    ...
    assert isinstance(coded_fragments, list), f"coded_fragments must be a list, not {type(coded_fragments)}"
    # assert len(coded_fragments) == STORAGE_NODES_NUM, f"len(coded_fragments) must be {STORAGE_NODES_NUM}, not {len(coded_fragments)}"
    # assert isinstance(coded_fragments[0], bytes), f"coded_fragments must be a list of bytes, not {type(coded_fragments[0])}"

    # assert data_req_socket.type == zmq.SUB, f"data_req_socket must be a ZMQ SUB socket, not {data_req_socket.type}"
    # assert response_socket.type == zmq.PULL, f"response_socket must be a ZMQ PULL socket, not {response_socket.type}"
    assert file_size > 0, f"file_size must be > 0, not {file_size}"
    ...
    for i in range(max_erasures):
        name_of_fragments.remove(random.choice(name_of_fragments))
