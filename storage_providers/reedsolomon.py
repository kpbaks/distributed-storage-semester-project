...
def store_file(file_data, max_erasures, send_task_socket, response_socket):
    ...
    for i in range(STORAGE_NODES_NUM):
        ...
        send_task_socket.send_multipart(
            [task.SerializeToString(), coefficients[:symbols] + bytearray(symbol)]
        )
...
def decode_file(symbols):
    ...
    # subtract the coefficients' size
    symbol_size = len(symbols[0]["data"]) - symbols_num
...
def get_file(
    coded_fragments, max_erasures, file_size, data_req_socket, response_socket
):
    """
...
def get_file_for_repair(
    fragments_to_retrieve, file_size, repair_socket, repair_response_socket
):
    """
    ...
    for name in fragments_to_retrieve:
        ...
        repair_socket.send_multipart(
            [b"all_nodes", header.SerializeToString(), task.SerializeToString()]
        )
    ...
    for file in files:
        ...
        if len(missing_fragments) > 0:
            ...
            if len(missing_fragments) > storage_details["max_erasures"]:
                print(
                    "Too many lost fragments: %s. Unable to repair file. "
